...
# Load in human spatial DRG data
spatial_drg <- readRDS(here('./data/price_spatial_transcriptomics/human.drg.combined.rds'))
spatial_drg[["percent.mt"]] <- PercentageFeatureSet(spatial_drg, pattern = "^MT-")
spatial_pig_qc <- VlnPlot(spatial_drg, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
head(spatial_drg@meta.data)
spatial_drg$labels <- Idents(spatial_drg)

# 8 separate ids (donors/batches)
...
# subset_genes_pig_ref <- subset(pig_ref, features = genes_intersection)
...
# subset_genes_spatial <- subset(spatial_drg, features = genes_intersection)
...
process_donor_data <- function(donor_id, full_query_data, reference_data, max_cells_per_batch = 250) {
  min_cells_per_batch <- 75
  reference_labels_df <- as_tibble(reference_data$labels, rownames = "cell_id") %>% rename(celltype = value)
  # Subset data for the specific donor
  donor_data <- subset(x = full_query_data, subset = id == donor_id)
  
  # Preprocessing steps
  # donor_data <- preprocess_data(donor_data) # Define this function as needed
  
  # Total number of cells and number of batches needed
  total_cells <- ncol(donor_data)
  ...
  for (batch_idx in 1:num_batches) {
    ...
    batch_cells <- colnames(donor_data)[start_idx:end_idx]
    batch_data <- subset(x = donor_data, cells = batch_cells)
    ...
    # integrated_data <- predict_cell_types(integrated_data, reference_data) # Define this function
  ...
  # combined_data <- rbind(processed_batches)
  combined_data <- do.call(rbind, processed_batches)
  combined_data$donor_id <- donor_id
...
# donor_ids <- c('pig1', 'pig2', 'pig3', 'pig4', 'pig5', 'pig6', 'pig7')
donor_ids <- unique(spatial_drg@meta.data$id)
# Parallel processing
processed_donors <- foreach(donor_id = donor_ids, .combine = 'rbind', .packages = c("Seurat")) %dopar% {
  process_donor_data(donor_id = donor_id, full_query_data = spatial, reference_data = pig_atlas, max_cells_per_batch = 250)
}

# Stop the parallel cluster
stopImplicitCluster()

write_csv(processed_donors, file = here(results_path, 'iterative_humanspatial_preds.csv'))